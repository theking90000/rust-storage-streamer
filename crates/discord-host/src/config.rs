...
#[derive(Parser)]
#[command(about = "Discord-webhook-backed file storage HTTP server")]
struct Cli {
...
impl Partial {
    fn from_env() -> Self {
        ...
        Self {
            ...
            object_rate: env_parse("DH_OBJECT_RATE"),
            data_ttfb_ms: env_parse("DH_DATA_TTFB_MS"),
            url_latency_ms: env_parse("DH_URL_LATENCY_MS"),
            frame_budget: env_parse("DH_FRAME_BUDGET"),
        }
    }
    ...
    fn from_toml(path: &PathBuf) -> Result<Self, BoxError> {
        ...
        toml::from_str(&text).map_err(|e| BoxError::from(format!("parsing config {}: {e}", path.display())))
    }
...
pub fn resolve() -> Result<Config, BoxError> {
    ...
    let config_path = env_str("DH_CONFIG").map(PathBuf::from).or_else(|| cli.config.clone());
