...
impl Catalog {
    ...
    pub async fn completed_file(&self, file_id: &str) -> Result<CompletedFile, CatalogError> {
        let row =
            sqlx::query("SELECT size, completed_at, name, content_type FROM files WHERE id = ?")
                .bind(file_id)
