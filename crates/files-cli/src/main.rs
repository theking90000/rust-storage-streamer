//! `streamer-files-cli` — upload a file or a piped stream to a
...
struct Cli {
    ...
    #[arg(
        ...
        long,
        env = "WD40_BACKEND",
        default_value = "https://wd40.theking90000.be"
    )]
    backend: String,
...
async fn main() -> Result<(), BoxError> {
    ...
    let response = client
        .post(format!("{base}/files/{id}/complete"))
        .send()
        .await?;
    if !response.status().is_success() {
        return Err(format!(
    ...
    for attempt in 0..=MAX_RETRIES {
        ...
        match result {
            Ok(response) if response.status().is_success() => {
                ...
                uploaded.fetch_add(
                    size.saturating_sub(counted.load(Ordering::Relaxed)),
                    Ordering::Relaxed,
                );
