...
the private `s3-gateway` API only; `files-gateway` is not mounted.
...
cargo run -p streamer-s3-discord -- credential grant ACCESS_KEY bucket
cargo run -p streamer-s3-discord -- credential grant ACCESS_KEY bucket --read-only

cargo run -p streamer-s3-discord -- credential revoke ACCESS_KEY
```

Use `--database-url` or `STREAMER_DATABASE_URL` to select another catalog.
...
`streamer-s3-discord serve --help` for rate and frame tuning options.
