...
use s3_gateway::{Catalog, DatabaseAuth, S3Storage};
...
enum Command {
    Serve {
        #[arg(long, env = "STREAMER_BIND", default_value = "0.0.0.0:8080")]
        bind: String,
        ...
        max_object_size: u64,
        #[arg(long, env = "STREAMER_TARGET_RATE", default_value_t = 60_000_000.0)]
        target_rate: f64,
        #[arg(long, env = "STREAMER_OBJECT_RATE", default_value_t = 60_000_000.0)]
        object_rate: f64,
...
async fn serve(
    ...
    let webhooks = discord_store::load_webhooks(&webhooks_file).await?;
    ...
    eprintln!(
        "streamer-s3-discord listening on http://{}",
        listener.local_addr()?
    );
